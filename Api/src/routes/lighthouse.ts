...
router.get('/companies', setLogConfig({ level: 'minimal' }), async (req, res) => {
  if (!req.auth) {
...
router.get('/companies/:companyId', setLogConfig({ level: 'minimal' }), async (req, res) => {
  if (!req.auth) {
    return res.status(401).json({ error: 'Unauthorized' });
  }
  ...
  return res.json({
    ...
    recentRuns: company.runs.map((run) => ({
      ...
      runner: run.runner ? { id: run.runner.id, name: `${run.runner.firstName} ${run.runner.lastName}` } : null,
    })),
  });
});
